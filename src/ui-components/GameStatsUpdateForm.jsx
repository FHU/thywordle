...
export default function GameStatsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = gameStatsRecord
      ...
      : initialValues;
    setWinDistribution(cleanValues.winDistribution);
    setGamesFailed(cleanValues.gamesFailed);
    setCurrentStreak(cleanValues.currentStreak);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(GameStats, idProp)
        : gameStats;
